...
const UI = {
  en: { cal: "Calendar", roteiro: "Itinerary", trans: "Transport", places: "Places", map: "Map",
    ...
    mapFallback: "The map needs internet to load (OpenStreetMap tiles). The other tabs work offline.",
  ...
  pt: { cal: "Calendário", roteiro: "Roteiro", trans: "Transportes", places: "Lugares", map: "Mapa",
    ...
    mapFallback: "O mapa precisa de internet para carregar (tiles do OpenStreetMap). As outras abas funcionam offline.",
  ...
  es: { cal: "Calendario", roteiro: "Itinerario", trans: "Transportes", places: "Lugares", map: "Mapa",
    ...
    mapFallback: "El mapa necesita internet para cargar (tiles de OpenStreetMap). Las otras pestañas funcionan sin conexión.",
  ...
  fr: { cal: "Calendrier", roteiro: "Itinéraire", trans: "Transports", places: "Lieux", map: "Carte",
    ...
    mapFallback: "La carte a besoin d’internet (tuiles OpenStreetMap). Les autres onglets fonctionnent hors ligne.",
  ...
  de: { cal: "Kalender", roteiro: "Reiseplan", trans: "Transport", places: "Orte", map: "Karte",
    ...
    mapFallback: "Die Karte braucht Internet (OpenStreetMap-Kacheln). Die anderen Tabs funktionieren offline.",
  ...
  it: { cal: "Calendario", roteiro: "Itinerario", trans: "Trasporti", places: "Luoghi", map: "Mappa",
    ...
    mapFallback: "La mappa ha bisogno di internet (tile OpenStreetMap). Le altre schede funzionano offline.",
...
export function renderTripHtml(trip) {
  ...
  const roteiroPanel = () => cells.map((c) => {
    ...
    const lis = c.items.map((it) => {
      const a = it.address ? `<div class="rt-addr">📍 ${bi(it.address)}</div>` : "";
  ...
  const placesPanel = () => {
    ...
    for (const g of groups) {
      ...
      for (const c of g.cells) {
        ...
        const lis = places.map((it) => `<li${it.type === "star" ? " class=\"star\"" : ""}>${itemInner(it)}${it.address ? `<div class="pl-addr">📍 ${bi(it.address)}</div>` : ""}</li>`).join("");
...
<script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js" crossorigin=""></script>`
  ...
  const mapPanel = hasMap ? `<div class="panel" data-tab="map"><div id="map" class="mapbox"><div class="map-fallback">${uiBi("mapFallback")}</div></div></div>` : "";
