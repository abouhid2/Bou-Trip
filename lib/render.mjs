...
export function buildDays(trip) {
  ...
  trip.stops.forEach((stop) => {
    ...
    for (let i = 0; i < nights; i++) {
      ...
      cells.push({ date, city: label, cityKey: stop.city, color, items, transit: isTransit });
    }
  });
  ...
  for (const c of cells) {
    ...
    for (const it of c.items) {
      if (it.coords && it.coords.length === 2) {
        markers.push({ lat: it.coords[0], lon: it.coords[1], name: it.text, idx, color: c.color, url: it.url || null, tickets: it.tickets || null, map: it.map || mapLinkFor(it.coords, provider) });
      }
    }
  }
...
<body>
  ...
  <div class="filter-hint">Toque numa cidade para ver só ela (vale pras 4 abas). "Mostrar tudo" limpa o filtro.</div>
