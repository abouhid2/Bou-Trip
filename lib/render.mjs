...
export function renderTripHtml(trip) {
  ...
  const photo = (it, cls) => it.image
    ? `<img class="${cls}" src="${esc(it.image)}" alt="${esc(tx(it.text, PRIMARY))}" loading="lazy" referrerpolicy="no-referrer" onerror="this.remove()">`
    : "";
  ...
  const placesPanel = () => {
    ...
    for (const g of groups) {
      ...
      for (const c of g.cells) {
        ...
        const lis = places.map((it) => `<li${it.type === "star" ? " class=\"star\"" : ""}>${itemInner(it)}${addrLink(it.address, "pl-addr")}${photo(it, "pl-img")}</li>`).join("");
  ...
  const langButtons = LANGS.map((lg) => `<button class="lang${lg === PRIMARY ? " active" : ""}" data-lang="${lg}">${esc(LANG_LABEL[lg] || lg.toUpperCase())}</button>`).join("");
  const appBar = `<div class="appbar"><div class="brand"><span class="brand-mark">✈️</span><span class="brand-name">Bou-Trip</span></div>${multi ? `<div class="langbar">${langButtons}</div>` : ""}</div>`;
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .pl-img { display: block; margin-top: 6px; width: 100%; max-width: 220px; height: 120px; object-fit: cover; border-radius: 8px; border: 1px solid #e6e6ee; background: #f0f0f4; }
  ...
  .rt-img { display: block; margin-top: 8px; width: 100%; max-width: 280px; height: 156px; object-fit: cover; border-radius: 10px; border: 1px solid #e6e6ee; background: #f0f0f4; }
